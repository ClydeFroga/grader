...
<script>
  export default {
    ...
    async asyncData({params, redirect}) {
      let cat = await fetch('http://promotech.igrader.ru/wp-json/wp/v2/mainthemes?search=' + params.slug)
      cat = await cat.json()
      if(cat.length === 0) {
        redirect(301, `/404`)
      }
      const url = 'http://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=' + cat[0].id;
