...
export const state = () => ({
  ...
  export const actions = {
    ...
    async load4({commit}) {
      let kruglyjStol = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=1604&per_page=4')
      kruglyjStol = await kruglyjStol.json()
      let krupniymPlanom = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=1603&per_page=5')
      krupniymPlanom = await krupniymPlanom.json()
      commit('setKruglyjStol', kruglyjStol)
      commit('setKrupniymPlanom', krupniymPlanom)
    },
    async load5({commit}) {
      let riders = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?tags=223&mainthemes__exclude=1599&per_page=7')
      riders = await riders.json()
      commit('setRiders', riders)
    },
    async load6({commit}) {
      let servismenyi = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=1605&per_page=4')
      servismenyi = await servismenyi.json()
      let stranitciIstorii = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=1606&per_page=5')
      stranitciIstorii = await stranitciIstorii.json()
      commit('setServismenyi', servismenyi)
      commit('setStranitciIstorii', stranitciIstorii)
    },
    async load7({commit}) {
      let vyborProfessionalov = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=1607&per_page=3')
      vyborProfessionalov = await vyborProfessionalov.json()
      commit('setVyborProfessionalov', vyborProfessionalov)
      let countOffers = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/mainthemes/1980')
      let r = rand(0, countOffers.count - 3)
      let specialoffers = await fetch('https://promotech.igrader.ru/wp-json/wp/v2/posts?mainthemes=1980&per_page=3&offset=' + r)
